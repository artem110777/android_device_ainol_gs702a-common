...
static void actions_server_DisplayService_cable_monitor(JNIEnv* env, jobject thiz) {
    int fd_de = -1;
    int ret_val = -1;
    int fd_max;
    fd_set fds;
    ...
    while (1) {
        ...
        if (FD_ISSET(fd_de, &fds)) {
            ...
            if (cable_state_hdmi > last_cable_state_hdmi) {
                ...
                env->CallVoidMethod(thiz, method_plugEvent, CABLE_HDMI_PLUG_OUT);
            }
...
/** helper APIs */
static inline int de_control_open(const struct hw_module_t* module, struct de_control_device_t** device) {
    return module->methods->open(module, DE_HARDWARE_MODULE_ID, (struct hw_device_t**) device);
}
...
static jboolean actions_server_DisplayService_init(JNIEnv *env, jclass clazz) {
    ...
    if (hw_get_module(HDMI_HARDWARE_MODULE_ID, (const hw_module_t**) &hdmimodule) == 0) {
        ...
        if (hdmi_dev_open(&hdmimodule->common, &hdmi_displayer.hdmi_device) == 0) {
            ALOGD("Got hdmi operations hdmi: %x", (unsigned int) hdmi_displayer.hdmi_device);
 ...
 */
static jint actions_server_DisplayService_getDisplayerCount(JNIEnv *env, jclass clazz) {
    return displayer_count;
}

static jstring actions_server_DisplayService_getDisplayerInfo(JNIEnv *env, jclass clazz, jint id) {
    displayer_t *dpy;
    char buf[256];
    ...
    sprintf(buf, "id=%d; width=%d; height=%d; name=%s", dpy->id, dpy->width, dpy->height, dpy->name);
    params8.append(buf);
    return env->NewStringUTF(params8);
}

static jstring actions_server_DisplayService_getHdmiCap(JNIEnv *env, jclass clazz) {
    ...
    if (status) return env->NewStringUTF("");
 ...
 *
 */
static jstring actions_server_DisplayService_getDisplayerParam(JNIEnv *env, jclass clazz) {
    char buf[512];
    String8 params8;

    // 2. push KEY_AUDIO_CHAN
    sprintf(buf, "colorkey=%s; color=%d; video_scale=%s; hdmi_res_width=%d; \
        hdmi_res_height=%d; hdmi_res_hz=%d; scale-x=%d; scale-y=%d",
        colorkey_enable, colorkey, video_scale, hdmi_res_width,
...
static jstring actions_server_DisplayService_getDisplayerParamStr(JNIEnv *env, jclass clazz, jstring displayer) {
    char buf[512];
    int hdmi_vid;
    String8 params8;
    ...
    jbyteArray barr = (jbyteArray) env->CallObjectMethod(displayer, mid, strencode);
    ...
    if (!strcmp("hdmi", str_id)) {
        if (hdmi_displayer.hdmi_device != 0) {
            hdmi_vid = hdmi_displayer.hdmi_device->get_vid(hdmi_displayer.hdmi_device);
...
static jboolean actions_server_DisplayService_setDisplayerParam(JNIEnv *env, jclass clazz, jstring params) {
    ...
    if (resWidth > 0 && resHeight > 0) {
        if (hdmi_displayer.hdmi_device != 0) {
            hdmi_displayer.hdmi_device->set_mode(hdmi_displayer.hdmi_device, resWidth, resHeight, resHz, resProgressive, resAspect);
        }
    }
    return true;
}

static void actions_server_DisplayService_setDisplayMode(JNIEnv *env, jclass clazz, jint mode) {
...
static void actions_server_DisplayService_setDisplaySingleMode(JNIEnv *env, jclass clazz, jint mode) {
...
static void actions_server_DisplayService_setSwitchStatus(JNIEnv *env, jclass clazz, jint status) {
...
static void actions_server_DisplayService_setHdmiDisconnectFlag(JNIEnv *env, jclass clazz, bool flag) {
...
static bool actions_server_DisplayService_closeOutputDisplayer(JNIEnv *env, jclass clazz, jint id) {
    int single_id;
...
static jboolean actions_server_DisplayService_setOutputDisplayer(JNIEnv *env, jclass clazz, jint id) {
    int j;
    int single_id;
    int drv_dpy_no;
    int sucess_id = 0;
    ...
    actions_server_DisplayService_closeOutputDisplayer(env, clazz, current_display);
...
static jboolean actions_server_DisplayService_setOutputDisplayer1(JNIEnv *env, jclass clazz, jstring id) {
    int j;
    int single_id;
    int drv_dpy_no;
...
static jstring actions_server_DisplayService_getOutputDisplayerStr(JNIEnv *env, jclass clazz) {
    String8 params8;
    int i;
 ...
 */
static jboolean actions_server_DisplayService_enableOutput(JNIEnv *env, jclass clazz, jboolean enable) {
    int single_id;
    if (current_display < 0) {
        return false;
    }
...
static bool actions_server_DisplayService_enableOutput1(JNIEnv *env, jclass clazz, jstring id, bool enable) {
    int ret;
    int i;
    ...
    result = strtok(str_id, delims);
    while (result != NULL) {
        if (!strcmp("hdmi", result)) {
            i = get_real_id(result);
            ret = hdmi_enable((hdmi_displayer_device_t *) displayers[i].opaque, enable);
...
static jint actions_server_DisplayService_getCableState(JNIEnv *env, jclass clazz) {
    int flag = 0;
...
int register_android_server_DisplayService(JNIEnv *env) {
    ...
    if (jniRegisterNativeMethods(env, "com/actions/server/DisplayService$CableMonitorThread",
        ...
        ALOGE("Register method to com/actions/server/DisplayService$CableMonitorThread failed");
        return -1;
    }
